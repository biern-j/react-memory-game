...
class App extends React.Component<{}, State> {
  constructor(props: {}) {
    ...
    this.state = {
      ...
      gameDiffculty: [
        { levelTitle: 'easy', multiple: 2, choosen: true },
        { levelTitle: 'medium', multiple: 3, choosen: false },
  ...
  async handleGameStartState(startGame: boolean) {
    ...
    if (startGame) {
      const dubledCards = await getDubleCards(
        this.state.players.length,
        levelDificulty.multiple,
      )

      this.setState({
        cards: sortRandomly(dubledCards),
      })
    }
  }
  ...
  handleGameRestart() {
    this.setState({ gameState: { ...this.state.gameState, start: !this.state.gameState.start, playersResults: [] } });
  }
      ...
      <GameContainer gameStart={this.state.gameState.start}>
        {!this.state.gameState.start ?
          <GameStart>
            ...
            <GameForm gameState={this.state} onSubmit={(value: { name: string; }) => this.onInputSubmit(value)} onLevelSetup={(level: string) => this.setLevelState(level)} onHandleGameStart={(gameStart: boolean) => this.handleGameStartState(gameStart)} onRemovePlayer={(palyerId: number) => this.handleRemovePlayer(palyerId)} />
          </GameStart>
          :
          
          
            <MemoryGame onNewGame={() => this.handleNewGame()} onGameRestart={() => this.handleGameRestart()} gameInfo={this.state} onSetToggle={(id) => this.setToggle(id)} onSetGameEnd={(cardsState) => setGameEnd(cardsState)} onHandleNewGame={() => this.handleNewGame()} />
        }
        <GlobalStyle />
      </GameContainer>
    )
...
const getDubleCards = async (totalPlayers: number, dificultyLevel: number) => {
  const singlePictures = await Promise.all(
    Array.from(new Array(totalPlayers * dificultyLevel)).map((item, index) =>
      fetchImage(`https://picsum.photos/200/300?random=${index}`),
    ),
  )
  const setDubleCards = (acc: Card[], curr: string): Card[] => {
  ...
  return singlePictures.reduce(setDubleCards, [])
}
